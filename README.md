...
2. Once you have enabled setup the project and created your OAuth ID client, download the client ID as `json` (look for the download option) and rename it `credentials.json`. NOTE: When creating the OAuth ID client you should see a list of diffrent credential types, check [this section](https://github.com/randomBrainstormer/MMM-GoogleCalendar#supported-oauth-credentials-type) to know which types are supported.
...
5. If you're using the `TV and unlimited input` as the OAuth credential type, run this command from the MMM-GoogleCalendar directory: `node authorize.js` and follow the instructions that will display in the console, otherwise you can safely skip this step as is not needed for other credential types. NOTE: After completing the `authorize.js` script it should print your calendar ID and most recent entries, you can use this to copy the ID of the calendar you want to config. (If you can't find yoru ID, check the troubleshooting section for help).

### Supported OAuth Credentials Type

As mentioned in the second step above, when creating your OAuth client ID you'll have to choose between a different set of options. This module only supports the `Web Application` and `TV and unlimited input` credential types. The main difference between between these is the authentication flow. With `TV and unlimited input` you can authenticate from the command line (useful for when using MagicMirror as server-only), while `Web Application` requires to use the same browser on which MagicMirror is being displayed.
...
**What's the difference between the `TV and unlimited input` and `Web Application` credential types?**
Check out the [dedicated section](https://github.com/randomBrainstormer/MMM-GoogleCalendar#supported-oauth-credentials-type) to OAuth credential types.
...
| When installing the module I get `TypeError: Cannot destructure property 'client_secret'..` | The credentials file from Google Cloud is of the wrong type, make sure to create a credential for `TV and unlimited input` or `Web Application`                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
